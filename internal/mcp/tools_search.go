...
func (s *Server) handleSearch(ctx context.Context, req CallToolRequest) (*CallToolResult, error) {
	_ = ctx

	intent, err := req.RequireString("intent")
	if err != nil {
		return NewToolResultError(fmt.Sprintf("missing required argument: %s", err.Error())), nil
	}
