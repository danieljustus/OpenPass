...
func (s *Server) handleSecureInput(ctx context.Context, req CallToolRequest) (*CallToolResult, error) {
	if !s.canWrite() {
		s.logAudit(ctx, "secure_input", "<write-denied>", false)
	...
	if err != nil {
		s.logAudit(ctx, "secure_input", "<invalid>", false)
		return NewToolResultError(err.Error()), nil
	}
	field, err := req.RequireString("field")
	if err != nil {
		s.logAudit(ctx, "secure_input", path, false)
		return NewToolResultError(err.Error()), nil
	}
	...
	s.logAudit(ctx, "secure_input", path, true)
