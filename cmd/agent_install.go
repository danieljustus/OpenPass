...
func writeInstallOutput(results []InstallResult) error {
	...
	default:
		for _, r := range results {
			...
			printQuietAware("✓ Agent %q configured\n", r.AgentName)
			printQuietAware("  Tier:         %s\n", r.Tier)
			printQuietAware("  Transport:    %s\n", r.Method)
			if r.ProfilePath != "" {
				printQuietAware("  Profile:      %s\n", r.ProfilePath)
			}
			if r.TokenID != "" {
			...
			if r.MCPConfigPath != "" {
				printQuietAware("  MCP config:   %s\n", r.MCPConfigPath)
			}
			if r.BackupPath != "" {
				printQuietAware("  Backup:       %s\n", r.BackupPath)
			}
			if r.SkillPath != "" {
				printQuietAware("  Skill:        %s\n", r.SkillPath)
			}
			printQuietAware("  Smoke test:   %s\n", r.SmokeTest)
		}
		return nil
	}
}
